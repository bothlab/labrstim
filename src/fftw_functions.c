...
int fftw_interface_theta_init(struct fftw_interface_theta* fftw_int)
{
  ...
  if((fftw_int->filter_function_theta=malloc(sizeof(double)*fftw_int->m))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filter_function_theta\n");
      return -1;
    }
  if((fftw_int->filter_function_delta=malloc(sizeof(double)*fftw_int->m))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filter_function_delta\n");
      return -1;
    }
  if((fftw_int->filtered_signal_theta=(double*) fftw_malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filtered_signal_theta\n");
      return -1;
    }
  if((fftw_int->filtered_signal_delta=(double*) fftw_malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filtered_signal_delta\n");
      return -1;
    }
  if((fftw_int->out_theta=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->out_theta\n");
      return -1;
    }
  if((fftw_int->out_delta=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * fftw_int->fft_signal_data_size))==NULL)
    {
  ...
  if((fftw_int->fft_plan_forward_theta=fftw_plan_dft_r2c_1d(fftw_int->fft_signal_data_size,fftw_int->filtered_signal_theta,fftw_int->out_theta,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_forward_theta\n");
      return -1;
    }
  if((fftw_int->fft_plan_backward_theta=fftw_plan_dft_c2r_1d(fftw_int->fft_signal_data_size, fftw_int->out_theta, fftw_int->filtered_signal_theta,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_backward_theta\n");
      return -1;
    }
  if((fftw_int->fft_plan_forward_delta=fftw_plan_dft_r2c_1d(fftw_int->fft_signal_data_size, fftw_int->filtered_signal_delta,fftw_int->out_delta,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_forward_delta\n");
      return -1;
    }
  if((fftw_int->fft_plan_backward_delta=fftw_plan_dft_c2r_1d(fftw_int->fft_signal_data_size,fftw_int->out_delta, fftw_int->filtered_signal_delta,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_backward_delta\n");
      return -1;
    }
...
int fftw_interface_theta_apply_filter_theta_delta(struct fftw_interface_theta* fftw_int)
{
  ...
  for(i=0; i < fftw_int->m;i++)
    {
      fftw_int->out_theta[i][0]=fftw_int->out_theta[i][0]*fftw_int->filter_function_theta[i];
      fftw_int->out_theta[i][1]=fftw_int->out_theta[i][1]*fftw_int->filter_function_theta[i];
      fftw_int->out_delta[i][0]=fftw_int->out_delta[i][0]*fftw_int->filter_function_delta[i];
      fftw_int->out_delta[i][1]=fftw_int->out_delta[i][1]*fftw_int->filter_function_delta[i];
    }
  // reverse the fft
  ...
  for (i=0;i <fftw_int->fft_signal_data_size;i++)
    {
      fftw_int->filtered_signal_theta[i]=fftw_int->filtered_signal_theta[i]/fftw_int->fft_signal_data_size;
      fftw_int->filtered_signal_delta[i]=fftw_int->filtered_signal_delta[i]/fftw_int->fft_signal_data_size;
    }
  return 0;
}
double fftw_interface_theta_delta_ratio(struct fftw_interface_theta* fftw_int)
{
  ...
  for (i=fftw_int->real_data_to_fft_size-fftw_int->power_signal_length;i < fftw_int->real_data_to_fft_size;i++)
    {
      fftw_int->signal_sum_square+=(fftw_int->filtered_signal_theta[i]*fftw_int->filtered_signal_theta[i]);
    }
  
  fftw_int->signal_mean_square=fftw_int->signal_sum_square/fftw_int->real_data_to_fft_size;
  fftw_int->signal_root_mean_square_theta=sqrt(fftw_int->signal_mean_square);

  // for delta
  fftw_int->signal_sum_square=0;
  for (i=fftw_int->real_data_to_fft_size-fftw_int->power_signal_length;i < fftw_int->real_data_to_fft_size;i++)
    {
      fftw_int->signal_sum_square+=(fftw_int->filtered_signal_delta[i]*fftw_int->filtered_signal_delta[i]);
    }
  fftw_int->signal_mean_square=fftw_int->signal_sum_square/fftw_int->real_data_to_fft_size;
  fftw_int->signal_root_mean_square_delta=sqrt(fftw_int->signal_mean_square);
  // check if the theta/delta ration is larger than threshold
  return fftw_int->signal_root_mean_square_theta/fftw_int->signal_root_mean_square_delta;
}
					   ...
					   double high_pass) // higher cut-off frequency
{
  ...
  if((function_low_pass = malloc(sizeof(double)*filter_length))==NULL)
    {
      fprintf(stderr,"unable to allocate memory for function_low_pass in fftw_interface_theta_make_butterworth_filter\n");
      return -1;
    }
  if((function_high_pass = malloc(sizeof(double)*filter_length))==NULL)
    {
      fprintf(stderr,"unable to allocate memory for function_high_pass in fftw_interface_theta_make_butterworth_filter\n");
      return -1;
    }
  ...
  for (i = 0; i < filter_length; i++)
    {      
      function_low_pass[i]= 1/sqrt(1+pow((frequency/high_pass),2*n_low));
...
double phase_difference(double phase1, double phase2)
{
  ...
  if(phase1<=phase2)
      ...
      if (phase_difference <= 180)
	{
      ...
      if (phase_difference > 180) // that means that 
	{
...
int fftw_interface_swr_init(struct fftw_interface_swr* fftw_int)
{
  int i;
  // the next 3 variables should be set according to argument pass to this function.
  // that way the size of signal analyzed could be set in main.c according to options
  ...
  if(fftw_int->fft_signal_data_size<fftw_int->real_data_to_fft_size)
    {
      fprintf(stderr,"fftw_int->fft_signal_data_size<fftw_int->real_data_to_fft_size in fftw_interface_swr_init\n");
      return -1;
    }
  fftw_int->m=fftw_int->fft_signal_data_size/2+1; // length of the fft complex array (diff than for numerical reciepe)
  ...
  fftw_int->frequency_wavelet_for_convolution=FREQUENCY_WAVELET_FOR_CONVOLUTION;
  ...
  if((fftw_int->ref_signal_data=malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->ref_signal_data\n");
      return -1;
    }
  if((fftw_int->wavelet_for_convolution=malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->wavelet_for_convolution\n");
      return -1;
    }
  for(i=0;i<fftw_int->fft_signal_data_size;i++)
    { // initialise to 0
  ...
  if((fftw_int->root_mean_square_array=malloc(sizeof(double)*fftw_int->size_root_mean_square_array))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->root_mean_square_array\n");
      return -1;
    }
  if((fftw_int->convolution_peak_array=malloc(sizeof(double)*fftw_int->size_root_mean_square_array))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->convolution_peak_array\n");
      return -1;
    }
  
  if((fftw_int->filter_function_swr=malloc(sizeof(double)*fftw_int->m))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filter_function_swr\n");
      return -1;
    }
  if((fftw_int->filtered_signal_swr=(double*) fftw_malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->filtered_signal_swr\n");
      return -1;
    }
  for (i = 0; i < fftw_int->fft_signal_data_size;i++)
    {fftw_int->filtered_signal_swr[i]=0;}// assigned to value of 0

  if((fftw_int->out_swr=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->out_swr\n");
      return -1;
    }
  if((fftw_int->out_wavelet=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->out_wavelet\n");
      return -1;
    }
  if((fftw_int->out_convoluted=(fftw_complex*) fftw_malloc(sizeof(fftw_complex) * fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->out_convoluted\n");
      return -1;
    }
  if((fftw_int->convoluted_signal=(double*) fftw_malloc(sizeof(double)*fftw_int->fft_signal_data_size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for fftw_int->convoluted_signal\n");
      return -1;
    }
  ...
  if((fftw_int->fft_plan_forward_swr=fftw_plan_dft_r2c_1d(fftw_int->fft_signal_data_size,fftw_int->filtered_signal_swr,fftw_int->out_swr,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_forward_swr\n");
      return -1;
    }
  // to fft the wavelet, the output complex of fftw has n/2+1 size , real is in out_swr[i][0] and imaginary is in out_swr [i][1]
  if((fftw_int->fft_plan_forward_wavelet=fftw_plan_dft_r2c_1d(fftw_int->fft_signal_data_size,fftw_int->wavelet_for_convolution,fftw_int->out_wavelet,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_forward_wavelet\n");
      return -1;
    }
  // the get back the filtered signal
  if((fftw_int->fft_plan_backward_swr=fftw_plan_dft_c2r_1d(fftw_int->fft_signal_data_size, fftw_int->out_swr, fftw_int->filtered_signal_swr,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_backward_swr\n");
      return -1;
    }

  // the get back the filtered signal
  if((fftw_int->fft_plan_backward_wavelet_convolution=fftw_plan_dft_c2r_1d(fftw_int->fft_signal_data_size, fftw_int->out_convoluted, fftw_int->convoluted_signal,FFTW_MEASURE))==NULL)
    {
      fprintf(stderr,"unable to create a plan for fftw_int->fft_plan_backward_swr\n");
      return -1;
    }
...
int fftw_interface_swr_differential_and_filter(struct fftw_interface_swr* fftw_int)
{
  ...
  for (i=0;i<fftw_int->real_data_to_fft_size;i++)
    {
      fftw_int->signal_data[i]=fftw_int->signal_data[i]-fftw_int->ref_signal_data[i];
      sum+=fftw_int->signal_data[i];
    }
  mean=sum/fftw_int->real_data_to_fft_size;
  ...
  for(i=0; i < fftw_int->m;i++)
    {
      fftw_int->out_swr[i][0]=fftw_int->out_swr[i][0]*fftw_int->filter_function_swr[i]*fftw_int->fft_scale;
      fftw_int->out_swr[i][1]=fftw_int->out_swr[i][1]*fftw_int->filter_function_swr[i]*fftw_int->fft_scale;
    }
...
double fftw_interface_swr_get_convolution_peak(struct fftw_interface_swr* fftw_int)
{
  ...
  if(fftw_int->number_convolution_peaks_analysed<fftw_int->size_root_mean_square_array)
    {
      fftw_int->convolution_peak_array[fftw_int->number_convolution_peaks_analysed]=max;
      
      // calculate the mean and sd with the data we have so far
      fftw_int->mean_convolution_peak=mean(fftw_int->number_convolution_peaks_analysed+1,fftw_int->convolution_peak_array,-1.0);
      fftw_int->std_convolution_peak=standard_deviation(fftw_int->number_convolution_peaks_analysed+1, fftw_int->convolution_peak_array,-1.0) ;
      fftw_int->number_convolution_peaks_analysed++;
    }
  return  (max-fftw_int->mean_convolution_peak)/fftw_int->std_convolution_peak;
}
...
double fftw_interface_swr_get_power(struct fftw_interface_swr* fftw_int)
{
  ...
  for (i=fftw_int->real_data_to_fft_size-fftw_int->power_signal_length;i < fftw_int->real_data_to_fft_size;i++)
    {
      fftw_int->signal_sum_square+=(fftw_int->filtered_signal_swr[i]*fftw_int->filtered_signal_swr[i]);
    }
  ...
  if(fftw_int->number_segments_analysed<fftw_int->size_root_mean_square_array)
    {
      fftw_int->root_mean_square_array[fftw_int->number_segments_analysed]=fftw_int->signal_root_mean_square;
      fftw_int->mean_power=mean(fftw_int->number_segments_analysed+1,fftw_int->root_mean_square_array,-9999999.0);
      fftw_int->std_power=standard_deviation(fftw_int->number_segments_analysed+1, fftw_int->root_mean_square_array,-1.0) ;
      fftw_int->number_segments_analysed++;
    }
  fftw_int->z_power=(fftw_int->signal_root_mean_square-fftw_int->mean_power)/fftw_int->std_power;
				   ...
				   double frequency)
{
  ...
  */
  ...
  if((wavelet=malloc(sizeof(double)*size))==NULL)
    {
      fprintf(stderr,"problem allocating memory for wavelet in function make_wavelet_for_convolution\n");
      return -1;
    }
  for (i = 0; i < size;i++) 
    {
